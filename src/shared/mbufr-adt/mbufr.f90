...
!}
  ...
  CHARACTER(LEN=25),PARAMETER                     ::MBUFR_VERSION=" 4.6.7 2022-06-23   "
...
subroutine INIT_MBUFR(verbose_in,logfile_in)
 ...
 !{ Variaveis da interface    
  ...
  !}
   ...
   IF (ERR < 0) THEN
    ...
    if (verbose>0) print *,TRIM(SUBNAME)//"_GET_OCTETS: Error: Unexpected number of bits => Diff=",err
     do k=na2,na
       A_VAL(K)=0
     end do
     NOCT2=NOCT
    ...
    do k = (APOS+1), NA2
      ...
      IF ((A_BITS(K)>0).AND.(A_BITS(K)<idigits)) THEN
        ...
        do b = 1,A_BITS(k) !' Varia do bit1 ao numero de bits de A_VAL(k)
          ...
          IF (i > noct2) THEN
	    if (verbose>0)  print *,"Erro  "//TRIM(SUBNAME)//"_GET_OCTETS: Unexpected number of bits"
            err=noct2-i
            return 
          END IF
...
!{ Interface
 ...
 !}
    ...
    do  k = 1, NA
      if (A_BITS(K)<0) then
        print *,TRIM(SUBNAME)//"_GET_OCTETS:  Fatal Error! Invalid number of bits"
        stop
      end if
...
!*********************************** 
 ...
 !-----------------------------------------------------------------------
   ...
   if (sec1%btype== 3) ndescmax= 500000
...
70    IF (err/=0) THEN
          errsec=err+50
          if (verbose>0) call ERRORLOG(un,errsec,desc_sec4,ifinal, "Error in expanddesc3")
          goto 99
      END IF
     ...
     !{
      IF ((errsec>0).and.(errsec<60)) THEN 
	   do j=1,sec3%nsubsets
		...
		END do
	   END do
       if (verbose>0) call ERRORLOG(un,errsec,desc_sec4,nvars, erromessage(errsec))
     END IF
   !}
...
!{ Declaracao das variaveis locais 
 ...
 INTEGER                            ::xx         ! Numero de octetos da secao 4 (menos os octetos iniciais) 
...
!{ Obtem o tamanho  
 ...
 !----------------------------------------------------
 !{
  xx=tam_sec4-3 ! Numero de octeros que conteem os descritores
...
!----------------------------------------------
 ...
 do i=1,nsubsets
    ...
    call tabc_setparm(err=err)
    IFinal=0     ! E zerado a cada subset, para re-iniciar a expansaao dos replicadores atrasado 
    ifinal_ant=0
...
444  	call expanddesc3(d,ndesc,ndxmax,dx,nvars,IFinal,err)
	if (err>0) then 
	    ...
	    errrd=err
         ...
         IF (xx*8<IFinal) THEN 
            if (verbose==3) print *,":MBUFR_ADT:This is strange: Section 4 has exceeded the espected sise. Size:",IFinal
            errrd=62
            IFinal=nvars
            deallocate (a,b,sec4,dx)
	     nsbr=i
   ...
   !{ No caso replicador atrasado de 1 bit, null = 1 
    ...
    !{ Caso o replicador seja missing entao assume vmax_numbits
     if (repfactor == null ) then 
       ...
       a(k)=repfactor
       if (verbose>0) call ERRORLOG(un,64,dx,ifinal, ERROMESSAGE(64),a,k0,i) 
       errrd=64 
       deallocate(a,b,sec4,dx)
       return     
    end if
      ...
      IF (d(j)%f==0) THEN
        ...
        !{Caso variavel numerica 
          ...
          if (d(j)%i==0) then 
            ...
            !{ Leitura do valor minimo (min value)   
		...
		if (verbose>0) then
		 ...
		 if (verbose>0) then 
            ...
            !  if (tabb(d(j)%f,d(j)%x,d(j)%y)%u==1) then 
            !     bbit=bbit*8 ! Caso seja variavel caracter entao converte bytes p/ bits
            ...
            !  end if
                ...
                IF (diff<-1000) THEN
                  ...
                  !{
                    print *,"Warning!  Compressed variable bigger than original variable"
       ...
       !{Se for descritor da tabelA B
        ...
        IF (d(j)%i==0) THEN
          ...
          !{ Processamento de variaveis caracteres (CCITT IA5 ) 
            ...
            !print *,"nbytes2,nbytes,j",nbytes2,nbytes,j
            
             !{Para todos os subsets sao obtidos todos os bytes
              do i=1,nsubset
                m=0
                do l=j,nbytes+j-1
                  m=m+1
...
!{ Copiando os descritores
   ...
   ! ao inves da copia simpes  
    ...
    if ((dc(i)%f==2).and.((dc(i)%x>22).and.(dc(i)%x<33)).and.(dc(i)%y==255).and.(i<ifinal)) then 
...
!
!    2-03-y   "O descritor subsequente define o novo valor
!              de referencia. Cada valor de referencia e 
!              definido por Y bits na secao de dados. Os
!              valores negativos saao definidos pelo bit1=1
      ...
      !{ Decodifica o valor Inteiro para REAL 
       ...
       if ((scale<=maxexp).and.(numbits_vint (iaux)<=rdigits)) then
         ...
         CVAL=REAL(IAUX,kind=realk)/real(dez**scale,kind=realk)
          ...
          print *,"Warning!  Error in CVAL during integer-real convertion"
...
!{ Variaveis da interface
   ...
   REAL(kind=realk),intent(in)::v              ! Valor REAL a ser convertido em BUFR
   TYPE(descbufr),intent(in)::d   ! Descritor correspondente
!}
...
SUBROUTINE INIT_ERROMESSAGE
    ...
    ERROMESSAGE(14)="Error reding BUFR Tables"
