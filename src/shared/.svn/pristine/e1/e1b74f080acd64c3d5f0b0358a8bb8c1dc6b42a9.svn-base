...
! Obtendo descritores das variaveis que representam mudanca de niveis verticais 
	nsiglev=0
	
	! Obtendo indentificador do tipo de nivel vertical 
	AUX1 =cindex("CAT")
	AUX2 =cindex("LEVEL")
	AUX3 =cindex("CHNM")
	IF (AUX1>0) then 
	...
	elseif (AUX2>0) then  
		! Obtendo nivel vertical de pressao
		do i=1,ncbtmax
			if (AUX2==cbt(i)%col) then 
				nsiglev=nsiglev+1
				siglev(nsiglev)=cbt(i)%d
			end if
		end do
	elseif(AUX3>0) then 
		! Obtendo canal do instrumento de radiancia 
